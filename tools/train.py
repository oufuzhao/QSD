...
def create_supervised_trainer(model, model_q, optimizer, criterion, loss_kd, cetner_loss_weight=0.0, device=None):
    """
...
#        print(batch)
        ...
        quality = model_q(img).data.cpu().numpy().squeeze()

        img2 = img2.to(device) if torch.cuda.device_count() >= 1 else img2
        score2, feat2, fea_list2 = model(img2)

        quality2 = model_q(img2).data.cpu().numpy().squeeze()

#        print('score',score.size())
#        print('feat',feat.size())
#        print('quality',quality.size())
#        print('quality2',quality2.size())


#        sys.exit()

#        weights = [10, 10, 10, 10, 10, 1]s
        fea_len = len(fea_list)
#        print('fea_len',fea_len)
        weights = list(np.ones(fea_len)*10)
        weights[-2] = 0.2
        weights[-1] = 0.2
        
        KD_loss, KD_losses = loss_kd(fea_list, fea_list2, quality, quality2, weights)
        KD_loss = KD_loss*0.6/4

        loss = criterion['total'](score, feat, target)*1.5/4
        # print(os.getcwd())
        # print('loss',loss)
        loss += KD_loss
...
def do_train(cfg, model, model_q, data_loader, optimizer, scheduler, criterion, loss_kd, num_query, start_epoch):
    ...
    trainer = create_supervised_trainer(
        model, model_q, optimizer, criterion, loss_kd, cfg.SOLVER.CENTER_LOSS_WEIGHT, device=device)
    ...
    if cfg.TEST.PARTIAL_REID == 'off':
        ...
        evaluator_reid = create_supervised_evaluator(model,
                                                     metrics={'r1_mAP_mINP': r1_mAP_mINP(
                                                         300, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)},
                                                     device=device)
        evaluator_ilids = create_supervised_evaluator(model,
                                                      metrics={'r1_mAP_mINP': r1_mAP_mINP(
                                                          119, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)},
                                                      device=device)
    ...
    def log_validation_results(engine):
        if engine.state.epoch % eval_period == 0:
            if cfg.TEST.PARTIAL_REID == 'off':
                ...
                logger.info("mAP: {:.112%}".format(mAP))
                for r in [1, 5, 10]:
