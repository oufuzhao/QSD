...
from .datasets import init_dataset, ImageDataset, ImageDataset_Test
...
def train_collate_fn(batch):
    imgs, pids, _, _, q_labels, imgs2, _, _, _, q_labels2,= zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)
    return torch.stack(imgs, dim=0), pids, q_labels, torch.stack(imgs2, dim=0), pids, q_labels2


def val_collate_fn(batch):
    imgs, pids, camids, imgpath = zip(*batch)
    return torch.stack(imgs, dim=0), pids, camids, imgpath
...
def make_data_loader(cfg):
    ...
    # load the quality annotations
    with open(cfg.DATASETS.QA_LABELS, 'r') as f: txtContent = f.readlines()
    quality_dict = {}
    for line in txtContent: 
        quality_dict[line.split()[0]] = float(line.split()[1])

    train_set = ImageDataset(cfg.DATASETS.NAMES, dataset.train, quality_dict, transforms['train'])
    ...
    if cfg.TEST.PARTIAL_REID == 'off':
        eval_set = ImageDataset_Test(dataset.query + dataset.gallery, transforms['eval'])
